...
export const infoCommand: Command = {
  ...
  execute: async (interaction) => {
    ...
    await interaction.editReply({ embeds: [await renderInfo(topic, id, page)] });
  },
...
async function renderAutoDetail(id: string): Promise<EmbedBuilder | null> {
  return (
    (await renderFishDetail(id)) ??
    (await renderRodDetail(id)) ??
    (await renderMapDetail(id)) ??
    (await renderPetDetail(id)) ??
    renderRecipeDetail(id) ??
    (await renderQuestDetail(id)) ??
    (await renderAchievementDetail(id)) ??
    (await renderSeasonDetail(id))
  );
}
...
async function renderRodDetail(id: string): Promise<EmbedBuilder | null> {
  const rod = await prisma.rod.findUnique({ where: { id } });
  if (!rod) return null;
...
async function renderMapDetail(id: string): Promise<EmbedBuilder | null> {
  const map = await prisma.map.findUnique({ where: { id }, include: { weatherState: true } });
  if (!map) return null;
