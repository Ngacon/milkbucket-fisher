...
const leaderboardCache = new Map<string, { embed: EmbedBuilder; expiresAt: number }>();
...
async function renderLeaderboard(type: string): Promise<EmbedBuilder> {
  const normalizedType = ['coins', 'fish', 'team'].includes(type) ? type : 'coins';
  const cached = leaderboardCache.get(normalizedType);
  if (cached && cached.expiresAt > Date.now()) {
    return EmbedBuilder.from(cached.embed);
  }

  const embed = await renderFreshLeaderboard(normalizedType);
  leaderboardCache.set(normalizedType, { embed, expiresAt: Date.now() + LEADERBOARD_CACHE_TTL_MS });
  return EmbedBuilder.from(embed);
}

async function renderFreshLeaderboard(type: string): Promise<EmbedBuilder> {
  if (type === 'team') {
